...
import { hasPendenciaClassificacao, parseNotaFiscalMetadata } from '../utils/notaFiscalMetadata';
...
type ProjetoCurvaResumo = {
  nome: string;
  dataInicio: Date | null;
  dataFim: Date | null;
  valorMonetarioPrevisto: unknown;
  valorMonetarioRealizado: unknown;
};

export class DashboardController {
  private readonly curvaProjetoLimit = 6;

  private async listarOrcadoNaoOrcadoAtivos(): Promise<string[]> {
    try {
  ...
  private isOrcadoClassificacao(value: unknown): boolean {
    const normalized = this.normalizeKey(value);
    return normalized === 'orcado';
  }

  private isNaoOrcadoClassificacao(value: unknown): boolean {
    const normalized = this.normalizeKey(value);
  ...
  private matchFilter(values: Array<unknown>, filters: string[]): boolean {
    if (!filters.length) {
      return true;
    }
  ...
  private calcularPessoasImpactadas(projetos: Array<{ impactoMensal: any; quantidadePessoasCadastradas: number }>): number {
    ...
    for (const projeto of projetos) {
      ...
      for (const item of impactoMensal) {
        ...
        const diretas = Number((item as any).pessoasDiretas || 0);
        const indiretas = Number((item as any).pessoasIndiretas || 0);
  ...
  private monthsBetweenInclusive(start: Date, end: Date): string[] {
  ...
  private buildCurvaMensalProjetos(
    projetos: ProjetoCurvaResumo[],
    projetoFilters: string[],
  ): {
    competencias: string[];
    series: Array<{ projeto: string; previsto: number[]; realizado: number[] }>;
    ...
    const seriesRaw = selected.map((projeto) => {
      ...
      const previstoByMonth = new Map<string, number>();
      const realizadoByMonth = new Map<string, number>();
    ...
    return {
      ...
      metodologia:
        'Valores totais previsto/realizado de cada projeto distribuídos igualmente entre os meses de início e fim do projeto.',
    };
  }

  async getMetricas(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      const metricas = await runWithPrismaFallback(
        async () => {
          const [
            notasFiscais,
            alertasAtivos,
            ...
            prisma.alerta.count({ where: { status: 'ATIVO' } }),
            prisma.instituicaoSocial.count(),
            prisma.obra.count(),
            prisma.obra.count({ where: { status: true } }),
            prisma.obra.count({ where: { status: false } }),
          ...
          const notasFiltradas = notasFiscais.filter((nota) => {
            const metadata = parseNotaFiscalMetadata(nota.observacao);
            ...
            const dataEmissao = new Date(nota.dataEmissao as any);
          ...
          const programasDisponiveisSet = new Set<string>(programasLista.map((item) => String(item.nome || '').trim()).filter(Boolean));
          const classificacoesDisponiveisSet = new Set<string>(
            classificacoesLista.map((item) => String(item.nome || '').trim()).filter(Boolean),
          );
          const orcadoDisponiveisSet = new Set<string>(
            orcadoNaoOrcadoAtivos.map((item) => String(item || '').trim()).filter(Boolean),
          );
          const projetosDisponiveisSet = new Set<string>(
            projetosAtivosLista.map((item) => String(item.nome || '').trim()).filter(Boolean),
          );
          ...
          const totalValor = notasFiltradas.reduce((acc, nf) => acc + Number((nf as any).valor || 0), 0);
          let totalValorOrcado = 0;
          let totalValorNaoOrcado = 0;

          const nfComPendenciaClassificacao = notasFiltradas.filter((item) =>
            hasPendenciaClassificacao(parseNotaFiscalMetadata(item.observacao)),
          ).length;

          const totalPessoasImpactadas = this.calcularPessoasImpactadas(projetosResumo);

          const nfPorObraMap = new Map<string, { name: string; value: number; valor: number }>();
          const nfPorProgramaMap = new Map<string, number>();
          const valorPorProgramaMap = new Map<string, number>();
          const nfPorClassificacaoMap = new Map<string, number>();
          ...
          for (const nf of notasFiltradas) {
            ...
            obraAtual.value += 1;
            obraAtual.valor += Number((nf as any).valor || 0);
            ...
            valorPorProgramaMap.set(programa, (valorPorProgramaMap.get(programa) || 0) + Number((nf as any).valor || 0));
            ...
            valorPorClassificacaoMap.set(
              classificacao,
              (valorPorClassificacaoMap.get(classificacao) || 0) + Number((nf as any).valor || 0),
            );
            ...
            const valorNF = Number((nf as any).valor || 0);
          ...
          const projetosCurva = projetosResumo
            ...
            .map((projeto) => ({
              projeto: projeto.nome,
              previsto: Number((projeto as any).valorMonetarioPrevisto || 0),
              realizado: Number((projeto as any).valorMonetarioRealizado || 0),
            }));

          const curvaPrevistoRealizadoMensalProjetos = this.buildCurvaMensalProjetos(
            projetosResumo as ProjetoCurvaResumo[],
            projetoFilters,
          );
        ...
        async () => {
          ...
          const nfComPendenciaClassificacao = fallback.notasFiscais.filter((nf) => {
            ...
            return hasPendenciaClassificacao(parseNotaFiscalMetadata(nf.observacao || null));
          }).length;

          for (const nf of fallback.notasFiscais) {
            const metadata = parseNotaFiscalMetadata(nf.observacao || null);
            ...
            const localizacao = String((nf as any)?.obra?.local || 'Sem localização').trim() || 'Sem localização';
            nfPorLocalizacaoMap.set(localizacao, (nfPorLocalizacaoMap.get(localizacao) || 0) + 1);
            valorPorLocalizacaoMap.set(localizacao, (valorPorLocalizacaoMap.get(localizacao) || 0) + valorNF);
          }
      ...
      res.status(200).json(successResponse(metricas.mensagem, metricas.payload));
    } catch (error: any) {
      res.status(500).json(errorResponse(error.message));
    }
  }

  async getAlertas(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      const page = parseInt(req.query.page as string) || 1;
      const pageSize = parseInt(req.query.pageSize as string) || 10;
      const skip = (page - 1) * pageSize;

      const alertasResponse = await runWithPrismaFallback(
        async () => {
      ...
      res.status(200).json(successResponse(alertasResponse.mensagem, alertasResponse.payload));
    } catch (error: any) {
      res.status(500).json(errorResponse(error.message));
    }
  }
