...
class NotaFiscalRepository {
    constructor() {
        this.fallbackFilePath = path_1.default.resolve(process.cwd(), 'data', 'notas-fiscais-fallback.json');
    }
    normalize(value) {
        return String(value || '')
            .normalize('NFD')
    ...
    parseDateBoundary(raw, endOfDay = false) {
        if (!raw) {
            return null;
        }
    ...
    stringFilterMatch(value, filters) {
        if (!filters?.length) {
            return true;
        }
    ...
    numberFilterMatch(value, filters) {
        if (!filters?.length) {
            return true;
        }
        return filters.includes(Number(value));
    }
    applyAdvancedFilters(items, filters, dataInicio, dataFim) {
        if (!filters) {
            return items;
        }
    ...
    async create(data) {
        const metadata = {
            camposObrigatorios: {
                ...(0, notaFiscalMetadata_1.emptyCamposObrigatorios)(),
        ...
        return (0, prismaCircuitBreaker_1.runWithPrismaFallback)(async () => {
            const notaFiscal = await database_1.default.notaFiscal.create({
                data: {
                    numeroNF: data.numeroNF,
                    fornecedor: data.fornecedor,
                    cnpj: data.cnpj,
                    valor: new library_1.Decimal(data.valor),
                    ...
                    observacao: (0, notaFiscalMetadata_1.encodeNotaFiscalMetadata)(metadata),
                },
    ...
    async findById(id) {
        return (0, prismaCircuitBreaker_1.runWithPrismaFallback)(async () => {
            const notaFiscal = await database_1.default.notaFiscal.findUnique({
                where: { id },
                include: this.defaultIncludes(),
            });
    ...
    async findAll(page = 1, pageSize = 10, filters) {
        ...
        return (0, prismaCircuitBreaker_1.runWithPrismaFallback)(async () => {
            const notasFiscais = await database_1.default.notaFiscal.findMany({
                where,
                orderBy: { dataCriacao: 'desc' },
                include: this.defaultIncludes(),
            });
            const filtrados = this.applyAdvancedFilters(notasFiscais.map((nf) => this.mapResponse(nf)), filters, dataInicio, dataFim);
            return {
    ...
    async update(id, data) {
        const atual = await (0, prismaCircuitBreaker_1.runWithPrismaFallback)(async () => database_1.default.notaFiscal.findUnique({ where: { id } }), async () => {
            const itens = await this.readFallback();
            return itens.find((item) => item.id === id) || null;
        });
        if (!atual) {
        ...
        if (data.valor !== undefined) {
            updateData.valor = new library_1.Decimal(data.valor);
        }
        if (data.dataEmissao !== undefined) {
            updateData.dataEmissao = new Date(data.dataEmissao);
        }
        const metadataAtual = (0, notaFiscalMetadata_1.parseNotaFiscalMetadata)(atual.observacao);
        if (data.camposClassificacao) {
            ...
            updateData.observacao = (0, notaFiscalMetadata_1.encodeNotaFiscalMetadata)(mergedMetadata);
            updateData.status = (0, notaFiscalMetadata_1.hasPendenciaClassificacao)(mergedMetadata) ? 'PENDENTE' : 'CLASSIFICADA';
        }
        else if (data.observacao !== undefined) {
            updateData.observacao = (0, notaFiscalMetadata_1.encodeNotaFiscalMetadata)({
                ...metadataAtual,
        ...
        return (0, prismaCircuitBreaker_1.runWithPrismaFallback)(async () => {
            const notaFiscal = await database_1.default.notaFiscal.update({
                where: { id },
                data: updateData,
                include: this.defaultIncludes(),
            });
    ...
    async delete(id) {
        await (0, prismaCircuitBreaker_1.runWithPrismaFallback)(async () => {
            await database_1.default.notaFiscal.delete({
                where: { id },
            });
        }, async () => {
            const itens = await this.readFallback();
    ...
    async deleteMany(ids) {
        ...
        return (0, prismaCircuitBreaker_1.runWithPrismaFallback)(async () => {
            const result = await database_1.default.notaFiscal.deleteMany({
                where: { id: { in: ids } },
            });
            return result.count;
        }, async () => {
    ...
    async findByNumero(numero) {
        return (0, prismaCircuitBreaker_1.runWithPrismaFallback)(async () => {
            const notaFiscal = await database_1.default.notaFiscal.findFirst({
    ...
    async getAllForPendenciaMetric() {
        return (0, prismaCircuitBreaker_1.runWithPrismaFallback)(async () => database_1.default.notaFiscal.findMany({
            select: {
                id: true,
                observacao: true,
            },
    ...
    mapResponse(notaFiscal) {
        const metadata = (0, notaFiscalMetadata_1.parseNotaFiscalMetadata)(notaFiscal.observacao);
        ...
        return {
            ...
            pendenteClassificacao: (0, notaFiscalMetadata_1.hasPendenciaClassificacao)({
                ...metadata,
                camposObrigatorios: {
                    orcadoNaoOrcado,
                    programa,
    ...
    async readFallback() {
        try {
            const raw = await promises_1.default.readFile(this.fallbackFilePath, 'utf-8');
    ...
    async writeFallback(data) {
        ...
        await promises_1.default.writeFile(this.fallbackFilePath, JSON.stringify(data, null, 2), 'utf-8');
    }
    async createFallback(data, metadata) {
        const itens = await this.readFallback();
        ...
        const registro = {
            ...
            pendenteClassificacao: (0, notaFiscalMetadata_1.hasPendenciaClassificacao)(metadata),
    ...
    async updateFallback(id, updateData) {
        const itens = await this.readFallback();
        ...
        const metadata = updateData.observacao
            ? (0, notaFiscalMetadata_1.parseNotaFiscalMetadata)(updateData.observacao)
            : {
                camposObrigatorios: {
        ...
        const atualizado = {
            ...
            actionCode: updateData.actionCode !== undefined ? updateData.actionCode : atual.actionCode ?? null,
            periodo: (() => {
                const dataEmissao = new Date((updateData.dataEmissao ? new Date(updateData.dataEmissao) : atual.dataEmissao));
                return Number.isNaN(dataEmissao.getTime())
                    ? ''
                    : `${String(dataEmissao.getMonth() + 1).padStart(2, '0')}/${dataEmissao.getFullYear()}`;
            })(),
            localizacao: atual.localizacao ?? null,
            observacao: metadata.textoObservacao || metadata.camposOpcionais.observacoes || null,
            camposClassificacao: {
                ...metadata.camposObrigatorios,
                ...metadata.camposOpcionais,
