...
interface CadastroResponsavelTecnicoFormProps {
  instituicoes: InstituicaoOption[];
  onSuccess?: () => void;
}

export const CadastroResponsavelTecnicoForm: React.FC<CadastroResponsavelTecnicoFormProps> = ({ instituicoes, onSuccess }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      await InstituicaoService.cadastrarResponsavelTecnico(form);
      setEnviado(true);
      setForm({
        instituicaoId: '',
        representante: '',
      ...
      alert('Erro ao cadastrar responsável técnico');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <form className="space-y-4" onSubmit={handleSubmit}>
      <h2 className="text-lg font-semibold text-slate-900 dark:text-white">Responsável Técnico</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <input name="representante" value={form.representante} onChange={handleChange} placeholder="Representante Técnico" className="w-full rounded-xl border border-slate-300 px-3 py-2 bg-white dark:bg-slate-800 dark:border-slate-700" required />
      ...
      <button type="submit" disabled={loading} className="rounded-xl bg-primary-600 px-4 py-2 font-semibold text-white hover:bg-primary-700 disabled:opacity-70">
        {loading ? 'Salvando...' : 'Cadastrar responsável técnico'}
      </button>

      {enviado && <p className="text-green-600 text-sm">Responsável técnico cadastrado com sucesso.</p>}
    </form>
  );
}
